...
public abstract class Trabalhador implements Comparable<Trabalhador> {
    ...
    /**
     * @param o
     * @return Definie comparação ascendente
     */
    @Override
    public int compareTo(Trabalhador o) {

        float esteVencimento = this.calcularVencimento();
        float outroVencimento = o.calcularVencimento();

        if (esteVencimento == outroVencimento) {
            return 0;
        } else if (esteVencimento < outroVencimento) {
            return -1;
        } else {
            return 1;
        }
